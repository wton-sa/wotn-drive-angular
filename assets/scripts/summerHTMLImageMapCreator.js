...
var summerHtmlImageMapCreator = (function () {
  ...
  function setShapeTitle(shape) {
    ...
    if (title) {
      const bbox = shape._el.getBBox();

      // Create the rectangle for background
      const rectElement = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
      const padding = 5; // Padding around the text
      rectElement.setAttribute('x', bbox.x + bbox.width / 2 - (title.length * 7) / 2 - padding); // Center the rectangle
      rectElement.setAttribute('y', bbox.y + 20 + bbox.height / 2 - 15); // Adjust for text height
      rectElement.setAttribute('width', (title.length * 7) + (padding * 2)); // Width based on title length
      ...
      textElement.setAttribute('y', bbox.y + 20 + bbox.height / 2);
