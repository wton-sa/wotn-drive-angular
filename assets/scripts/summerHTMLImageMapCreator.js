...
var summerHtmlImageMapCreator = (function () {
  ...
  var utils = {
    ...
    getOffset: function (node) {
      ...
      return {
        x: Math.round(boxCoords.left + window.pageXOffset + document.getElementById('main-w').scrollLeft),
        y: Math.round(boxCoords.top + window.pageYOffset + document.getElementById('main-w').scrollTop)
      };
    },
    ...
    getRightCoords: function (x, y) {
      return {
        x: x - app.getOffset('x') + document.getElementById('main-w').scrollLeft,
        y: y - app.getOffset('y') + document.getElementById('main-w').scrollTop
      };
    },
  ...
  var app = (function () {
    ...
    return {
      ...
      setDimensions: function (width, height) {
        ...
        domElements.heade.style.height = height + 'px';
        return this;
      },
  ...
  function Area(type, coords, attributes) {
     ...
     * @property desc {string} - desc-attribute of area
     */
    this._attributes = {
  ...
  Area.ATTRIBUTES_NAMES = ['ID', 'HREF', 'ALT', 'TITLE', 'DESC'];
  ...
  /**
   * Set attributes (id, href, alt , title and desc) for this area
   * 
  ...
  Area.prototype.setInfoAttributes = function (attributes) {
    this._attributes.id = attributes.id ?? getUniqueNumber().toString();
    this._attributes.href = attributes.href;
    this._attributes.alt = attributes.alt;
    this._attributes.title = attributes.title;
    this._attributes.desc = attributes.desc;
    const shape = this;
    setTimeout(function () { setShapeTitle(shape); }, 100);
  ...
  function removeShape(shape) {
    ...
    const existingText = svg.querySelector(`text[data-shape-id="${shape._attributes.id}"]`);
    if (existingText) {
      existingText.remove();
    }
    const existingRect = svg.querySelector(`rect[data-shape-id="${shape._attributes.id}"]`);
    if (existingRect) {
      existingRect.remove();
    }
    shape.remove();
  ...
  function setShapeTitle(shape) {
    const svg = document.getElementById('svg');

    // Remove existing text and rectangle
    const existingText = svg.querySelector(`text[data-shape-id="${shape._attributes.id}"]`);
    if (existingText) {
      existingText.remove();
    }
    ...
    const title = shape._attributes.title;
    if (title) {
      ...
      if (shape._type === 'rectangle') {
        x = shape._coords.x + shape._coords.width / 2 - (title.length * 7) / 2 - padding;
        y = shape._coords.y + 20 + shape._coords.height / 2 - 15; // Adjust for text height
      } else if (shape._type === 'circle') {
        x = shape._coords.cx - (title.length * 7) / 2 - padding; // Center based on cx
        y = shape._coords.cy + 20; // Adjust for text height
      } else if (shape._type === 'polygon') {
        // Calculate centroid of the polygon
        const points = shape._coords.points;
        let centroidX = 0, centroidY = 0;
        points.forEach(point => {
          centroidX += point.x;
          centroidY += point.y;
        });
        centroidX /= points.length; // Average for x
        centroidY /= points.length; // Average for y

        x = centroidX - (title.length * 7) / 2 - padding; // Center based on centroid
        y = centroidY + 20; // Adjust for text height
      }

      // Set rectangle dimensions
      const rectWidth = (title.length * 7) + (padding * 2);
      const rectHeight = 20; // Set a fixed height for the rectangle
      rectElement.setAttribute('x', x);
      rectElement.setAttribute('y', y - rectHeight + 20); // Adjust for vertical alignment
      rectElement.setAttribute('width', rectWidth);
      rectElement.setAttribute('height', rectHeight); // Set height
      rectElement.setAttribute('class', 'title-badge-container');
      rectElement.setAttribute('data-shape-id', shape._attributes.id);

      // Append the rectangle to the group
      shape._groupEl.appendChild(rectElement);

      // Create the text element
      const textElement = document.createElementNS('http://www.w3.org/2000/svg', 'text');
      textElement.textContent = title;
      textElement.setAttribute('x', x + rectWidth / 2); // Center within the rect
      textElement.setAttribute('y', y + 15); // Align with the rectangle


      textElement.setAttribute('class', 'title-badge');
      textElement.setAttribute('data-shape-id', shape._attributes.id);

      // Append the text element to the group
      shape._groupEl.appendChild(textElement);
    }
  }
  ...
  Area.copy = function (originalArea) {
    var newAreaJson = JSON.parse(JSON.stringify(originalArea.toJSON()));
    newAreaJson.attributes.id = getUniqueNumber().toString(); // set new id for copy
    //console.log('Id from: ' + originalArea._attributes.id + '  ===> ' + newAreaJson.attributes.id);
    return Area.fromJSON(newAreaJson).move(10, 10).select();
  };
  ...
  Rectangle.prototype.setSVGCoords = function (coords) {
    ...
    this._el.setAttribute('title', this._attributes?.title);
  ...
  Rectangle.prototype.toHTMLMapElementString = function () {
    ...
    return '<area shape="rect" coords="' // TODO: use template engine
      ...
      (this._attributes.desc ? ' desc="' + this._attributes.desc + '"' : '') +
      ' />';
  };
  ...
  Circle.prototype.toHTMLMapElementString = function () {
    return '<area shape="circle" coords="' +
      ...
      (this._attributes.alt ? ' alt="' + this._attributes.alt + '"' : '') +
      (this._attributes.title ? ' title="' + this._attributes.title + '"' : '') +
      (this._attributes.desc ? ' title="' + this._attributes.desc + '"' : '') +
      ' />';
  };
  ...
  var info = (function () {
    var form = utils.id('edit_details'),
      ...
      temp_x,
      temp_y;
    if (obj) href_attr.value = obj.href;
    ...
    function setCoords() {
      ...
      const left = (viewportWidth - form.offsetWidth) / 2;
      const top = (viewportHeight - form.offsetHeight) / 2;

      // Set the form's position
      form.style.left = (left - 300) + 'px';
      form.style.top = (top - 300) + 'px';
    }
    ...
    desc_attr.addEventListener('keydown', function (e) {
      e.stopPropagation();
    }, false);
    ...
    desc_attr.addEventListener('change', change, false);
    ...
    return {
      load: function (object) {
        ...
        desc_attr.value = object._attributes.desc ? object._attributes.desc : '';
